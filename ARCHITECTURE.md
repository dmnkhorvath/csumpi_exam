...
> A comprehensive guide to recreating the Hungarian medical exam question processing platform via agentic coding.
...
The platform processes Hungarian medical qualification exam PDFs through a multi-stage pipeline:
...
│  3. categorize_questions.py (Gemini AI)      │  JSON → 11 medical categories
...
**Scale:** ~327 exam PDFs, 5,000+ extracted questions, 11 medical categories.
...
2. Sends each image to Gemini with a system prompt that explains the Hungarian medical exam format:
...
**Purpose:** Classify every parsed question into one of 11 Hungarian medical categories using Gemini AI.
...
1. Merges all `parsed.json` files from the extraction stage into a single list.
2. For each question, sends `question_text` + `correct_answer` to Gemini with the list of valid categories.
3. Gemini returns a JSON response with `category` and `reasoning`.
4. Uses response schema validation to constrain output to valid categories.
5. Processing is parallel (10 workers default) and thread-safe with incremental saving every 10 questions.
6. Produces a category distribution report upon completion.
...
| `categorize_question(question, client)` | Single Gemini call returning `{category, reasoning}` |
...
1. Group all questions by their medical category (so similarity is only measured within the same topic).
...
| `group_by_category(questions)` | Partition questions by medical category |
...
1. Reads `public/categorized_questions_with_similarity.json`.
2. Groups questions by category.
3. Within each category, groups questions by `similarity_group_id`.
4. Questions without a similarity group (null) get a synthetic ID `__null_{index}` so each becomes its own single-item group.
5. Sorts groups by size (largest first).
6. Transliterates Hungarian characters in filenames (á→a, é→e, etc.) using `sanitize_filename()`.
7. Writes one JSON file per category to `public/categories/`.
...
- Renders a responsive grid (1/2/3 columns by breakpoint) of all 11 categories.
- Each category is a link styled with DaisyUI `btn-outline-lg`.
- Categories come from `src/helpers/categories.js`.
...
- Loads all 11 category JSON files in parallel.
...
#### Category Definitions (`src/helpers/categories.js`)

Exports a `Categories` object mapping constant keys to `{name, file}` pairs:

```javascript
export const Categories = {
  A_MOZGAS_SZERV_RENDSZERE: {
    name: 'A mozgás szerv rendszere',
    file: 'a_mozgas_szerv_rendszere.json'
  },
  // ... 10 more entries
}
```

The `file` values correspond to the sanitized filenames produced by `split_by_category.py`.
...
public/categories/*.json
    │
    ├─── HomePage fetches nothing (static category list)
    │
    ├─── CategoryPage fetches /categories/{name}.json
    │    └─ Renders groups with best-question selection
    │
    └─── SimilarityGroupsPage fetches ALL /categories/*.json
...
2. Write a system prompt that explains Hungarian medical exam format (red text = answers, black text = questions, markdown tables).
...
### Phase 4: Categorization Script

1. Create `scripts/categorize_questions.py`.
2. Define the 11 Hungarian medical categories (see Section 2, Step 3).
3. Implement `merge_parsed_files()` to combine all `parsed.json` files.
4. Send each question's text + answer to Gemini with category list.
5. Use response schema validation to constrain output.
6. Process in parallel (10+ workers) with thread-safe incremental saving.
7. Output: `data/categorized_questions.json`.

### Phase 5: Similarity Detection Script
...
### Phase 6: Category Split Script

1. Create `scripts/split_by_category.py`.
2. Read the similarity-enriched JSON.
3. Group by category, then by `similarity_group_id`.
4. Assign `__null_{index}` IDs to ungrouped questions.
5. Sort groups by size (descending).
6. Transliterate Hungarian characters for filenames.
7. Write one JSON per category to `public/categories/`.

### Phase 7: Frontend — Entry Point & Routing
...
4. Create `src/helpers/categories.js` with the 11 category definitions (name + filename).

### Phase 8: Frontend — HomePage

1. Create `src/pages/HomePage.jsx`.
2. Render responsive grid of category buttons from `Categories` object.
3. Each button links to `/category/{file_without_extension}`.
4. Add secondary link to `/similarity-groups`.

### Phase 9: Frontend — CategoryPage
...
### Phase 10: Frontend — SimilarityGroupsPage

1. Create `src/pages/SimilarityGroupsPage.jsx`.
2. Fetch all 11 category JSONs in parallel.
...
### Phase 12: End-to-End Pipeline Run
...
# 3. Categorize into 11 medical topics
uv run scripts/categorize_questions.py data/extracted_questions/ -o data/categorized_questions.json -w 20
...
# 6. Split into per-category files for frontend
uv run scripts/split_by_category.py
